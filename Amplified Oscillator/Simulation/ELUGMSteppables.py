...
#Self-Cutoff
ENDMCS=400000 #call runtime here directly
#Mitosis Variables
...
#                 print CSAY, CSAG, PTSG, CSAB, PTSB, CSAR, PTSR, CSAM
            ...
            if (cell.type==1 or cell.type==2 or cell.type==3 or cell.type==6):
                DTREPO=(1/(1+np.exp(-((PTSG+PTSW)-BETAONE))))-(1/KAPPA)*cell.dict["PTS"][1]         #RFP amplifer (not shown)       GRBY 0123
                cell.dict["PTS"][1]+=DTREPO
                DTREPT=(1/(1+np.exp(-((cell.dict["PTS"][1])-BETATWO))))-(1/KAPPA)*cell.dict["PTS"][2]         #BFP       GRBY 0123
                cell.dict["PTS"][2]+=DTREPT                                                          
                DTREPH=(1/(1+np.exp(((cell.dict["PTS"][2])-BETATHREE))))-(1/KAPPA)*cell.dict["PTS"][0]    #GFP 
                cell.dict["PTS"][0]+=DTREPH                                                                                                      
...
#            self.pW4.addDataPoint("YFP", mcs, YFPPTS/(NUMTO+NUMTL+NUMTR+NUMTK))
            self.pW4.addDataPoint("RFP", mcs, RFPPTS/(NUMTY+NUMTG+NUMTB+NUMTW))
